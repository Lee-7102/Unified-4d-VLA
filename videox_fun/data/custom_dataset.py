...
def load_video_frames(video_path, sample_n_frames, sample_stride,
                      length_drop_start, length_drop_end, resize_size, 
                      enable_bucket, video_transforms, timeout, idx):
        """Helper to load, resize, and process frames from a video file."""
        with VideoReader_contextmanager(video_path, num_threads=2) as video_reader:
            min_sample_n_frames = min(
                sample_n_frames,
                int(len(video_reader) * (length_drop_end - length_drop_start) // sample_stride)
            )
            if min_sample_n_frames == 0:
                raise ValueError(f"No Frames in video: {video_path}")

            video_length = int(length_drop_end * len(video_reader))
            clip_length = min(video_length, (min_sample_n_frames - 1) * sample_stride + 1)
            start_idx = random.randint(
                int(length_drop_start * video_length),
            ...
            try:
                ...
                pixel_values = func_timeout(timeout, get_video_reader_batch, args=sample_args)

                resized_frames = [resize_frame(frame, resize_size) for frame in pixel_values]
                pixel_values = np.array(resized_frames)

            except FunctionTimedOut:
                raise ValueError(f"Read timeout for {video_path}.")
            except Exception as e:
                raise ValueError(f"Failed to extract frames from {video_path}. Error: {e}")
...
class ImageVideoDataset(Dataset):
    ...
    def get_batch(self, idx):
        ...
        if data_info.get('type', 'image')=='video':#'image is the return default
            video_id, pointmap_id, ee_id, text = data_info['file_path'], data_info['pointmap_path'], data_info['ee_path'],data_info['text']
            ...
            if self.data_root is None:
                ...
                ee_dir = os.path.join(self.data_root, ee_id)
            ...
            pixel_ee_values = load_video_frames(
                ee_dir,
                self.video_sample_n_frames,
                self.video_sample_stride,
                self.video_length_drop_start,
            ...
            pixel_values = torch.cat(
                [pixel_video_values, pixel_pointmap_values, pixel_ee_values],
    ...
    def __getitem__(self, idx):
        ...
        while True:
            ...
            try:
                ...
                sample["pixel_values"] = pixel_values #15f*3*h*w
                sample["text"] = name
